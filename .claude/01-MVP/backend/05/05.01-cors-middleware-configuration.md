...
- [ ] `CORS_EXPOSE_HEADERS` environment variable is parsed correctly
- [ ] Comma-separated format works: `X-Request-ID,X-RateLimit-Limit`
- [ ] JSON array format works: `["X-Request-ID", "X-RateLimit-Limit"]`
- [ ] Default value includes all four required headers
- [ ] Empty string results in empty list (no exposed headers)
- [ ] Whitespace is properly trimmed from header names

### Middleware Integration

- [ ] `expose_headers` parameter added to CORSMiddleware registration
- [ ] Preflight (OPTIONS) requests include `Access-Control-Expose-Headers`
- [ ] Actual requests include `Access-Control-Expose-Headers`
- [ ] Frontend JavaScript can read `X-Request-ID` from response headers

### Production Validation

- [ ] Warning logged for wildcard origin with credentials
- [ ] Warning logged for empty origins in production
- [ ] Warning logged for HTTP (non-localhost) origins in production
- [ ] No warnings for valid production configuration
- [ ] Localhost HTTP origins allowed (for testing purposes)

### Backward Compatibility

- [ ] Existing `CORS_ORIGINS` parsing unchanged
- [ ] Existing `CORS_ALLOW_METHODS` parsing unchanged
- [ ] Existing `CORS_ALLOW_HEADERS` parsing unchanged
- [ ] Existing `CORS_ALLOW_CREDENTIALS` behavior unchanged
- [ ] No breaking changes to existing .env files

### Documentation

- [ ] `.env.example` updated with `CORS_EXPOSE_HEADERS`
- [ ] Debug endpoint shows `expose_headers` configuration
- [ ] Startup logs CORS validation warnings

### Testing

- [ ] Unit tests for `cors_expose_headers` property (5+ test cases)
- [ ] Unit tests for `validate_cors_for_production` (5+ test cases)
- [ ] Integration tests for CORS header behavior (3+ test cases)
- [ ] All existing CORS-related tests still pass
