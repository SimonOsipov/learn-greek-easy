...
### Changes Made
- Added `create_card` endpoint to `src/api/v1/cards.py`
- Added required imports: `status`, `get_current_superuser`, `User`, `CardCreate`
- Implemented deck existence validation (allows inactive decks for admin)
- Added comprehensive OpenAPI documentation

### Tests Added
- 14 integration tests in `tests/integration/api/test_cards.py`
- All tests passing

### PR: https://github.com/SimonOsipov/learn-greek-easy/pull/32
...
- **All tests passing**: 14/14 create card endpoint tests
- **Full cards API tests passing**: 39/39

### API Contract Compliance

| Requirement | Status |
|-------------|--------|
| POST /api/v1/cards endpoint | PASS |
| Returns 201 Created on success | PASS |
| Uses `get_current_superuser` dependency | PASS |
| Validates deck exists before creating | PASS |
| Uses CardRepository.create() pattern | PASS |
| Commits transaction properly | PASS |
| Has proper OpenAPI documentation | PASS |
| Follows existing code patterns from decks.py | PASS |

### Validation Rules

| Field | Requirement | Status |
|-------|-------------|--------|
| deck_id | Required, valid UUID | PASS |
| front_text | Required, min 1 char | PASS |
| back_text | Required, min 1 char | PASS |
| example_sentence | Optional | PASS |
| pronunciation | Optional, max 255 chars | PASS |
| difficulty | Required, enum | PASS |
| order_index | Optional, default 0, >= 0 | PASS |
...
|------|-----------|--------|
| 201 | Success | PASS |
| 401 | Not authenticated | PASS |
| 403 | Not superuser | PASS |
| 404 | Deck not found | PASS |
| 422 | Validation errors | PASS |

### Test Coverage (14 tests)

1. test_create_card_success - PASS
2. test_create_card_minimal_fields - PASS
3. test_create_card_unauthenticated_returns_401 - PASS
4. test_create_card_non_superuser_returns_403 - PASS
5. test_create_card_deck_not_found_returns_404 - PASS
6. test_create_card_validation_error_missing_front_text_returns_422 - PASS
7. test_create_card_validation_error_missing_back_text_returns_422 - PASS
8. test_create_card_validation_error_invalid_difficulty_returns_422 - PASS
9. test_create_card_validation_error_invalid_deck_id_format_returns_422 - PASS
10. test_create_card_validation_error_negative_order_index_returns_422 - PASS
11. test_create_card_all_difficulties - PASS
12. test_create_card_in_inactive_deck_succeeds - PASS
13. test_created_card_is_persisted - PASS
14. test_create_card_response_fields - PASS

### Design Requirements

| Requirement | Status |
|-------------|--------|
| Allow creating cards in inactive decks (admin privilege) | PASS |
| Card is persisted and retrievable | PASS |
| Response includes all CardResponse fields | PASS |
