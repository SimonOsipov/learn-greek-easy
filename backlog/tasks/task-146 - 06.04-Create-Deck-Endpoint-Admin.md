...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 POST /api/v1/decks endpoint implemented
- [x] #2 Returns 201 Created with created deck
- [x] #3 Returns 401 if not authenticated
- [x] #4 Returns 403 if authenticated but not superuser
- [x] #5 Validates DeckCreate schema (name required, valid level)
- [x] #6 Sets is_active=True by default
- [x] #7 Generates UUID for new deck
- [x] #8 Sets created_at and updated_at timestamps
- [x] #9 Commits transaction properly
...
<!-- SECTION:NOTES:BEGIN -->
PR: https://github.com/SimonOsipov/learn-greek-easy/pull/22
...
- **Date**: 2025-12-07
- **Status**: PASSED
- **Commit**: e4813b4 - [task-146] Implement POST /api/v1/decks endpoint for admin deck creation

### Checks Performed

#### 1. Code Review
- [x] POST endpoint implemented at `/api/v1/decks` (lines 109-168 in `src/api/v1/decks.py`)
- [x] Uses `status.HTTP_201_CREATED` response code
- [x] Requires superuser authentication via `get_current_superuser` dependency
- [x] Validates `DeckCreate` schema (name: str 1-255 chars, level: DeckLevel enum)
- [x] Transaction handling correct: `repo.create()` + `db.commit()` + `db.refresh()`
- [x] Returns `DeckResponse` with proper model validation

#### 2. Schema Verification (`src/schemas/deck.py`)
- [x] `DeckCreate` schema exists (lines 30-33)
- [x] Inherits from `DeckBase` with required fields: `name`, `level`
- [x] `description` is optional (from DeckBase)
- [x] Name validation: min_length=1, max_length=255
- [x] Level uses `DeckLevel` enum (A1, A2, B1, B2, C1, C2)

#### 3. Authentication/Authorization (`src/core/dependencies.py`)
- [x] `get_current_superuser` dependency exists (lines 105-134)
- [x] Properly chains `get_current_user` and checks `is_superuser`
- [x] Raises `ForbiddenException` (403) for non-superusers
- [x] `get_current_user` raises 401 for missing/invalid tokens

#### 4. Repository (`src/repositories/deck.py` + `base.py`)
- [x] `DeckRepository` inherits from `BaseRepository[Deck]`
- [x] `BaseRepository.create()` method handles Pydantic schema conversion
- [x] Uses `session.add()` + `flush()` pattern (commit deferred to endpoint)

#### 5. Model Verification (`src/db/models.py`)
- [x] `Deck` model with UUID primary key (`server_default=func.uuid_generate_v4()`)
- [x] `is_active` default=True (line 287)
- [x] Uses `TimestampMixin` for `created_at` and `updated_at`
- [x] Timestamps use `server_default=func.now()` and `onupdate=func.now()`

#### 6. Test Coverage
- [x] 11 integration tests created in `tests/integration/api/test_decks.py::TestCreateDeckEndpoint`
- [x] All tests pass successfully

**Tests Executed**:
```
test_create_deck_success - PASSED
test_create_deck_without_description - PASSED
test_create_deck_all_levels - PASSED
test_create_deck_unauthenticated_returns_401 - PASSED
test_create_deck_non_superuser_returns_403 - PASSED
test_create_deck_missing_name_returns_422 - PASSED
test_create_deck_missing_level_returns_422 - PASSED
test_create_deck_invalid_level_returns_422 - PASSED
test_create_deck_empty_name_returns_422 - PASSED
test_create_deck_name_too_long_returns_422 - PASSED
test_created_deck_is_persisted - PASSED
```
...
|---|-----------|--------|----------|
| 1 | POST /api/v1/decks endpoint implemented | PASS | `src/api/v1/decks.py` lines 109-168 |
| 2 | Returns 201 Created with created deck | PASS | `status_code=status.HTTP_201_CREATED`, test: `test_create_deck_success` |
| 3 | Returns 401 if not authenticated | PASS | `get_current_superuser` -> `get_current_user` raises 401, test: `test_create_deck_unauthenticated_returns_401` |
| 4 | Returns 403 if authenticated but not superuser | PASS | `get_current_superuser` raises ForbiddenException, test: `test_create_deck_non_superuser_returns_403` |
| 5 | Validates DeckCreate schema (name required, valid level) | PASS | Tests: `test_create_deck_missing_name_returns_422`, `test_create_deck_invalid_level_returns_422` |
| 6 | Sets is_active=True by default | PASS | `Deck` model default=True, test: `test_create_deck_success` asserts `is_active is True` |
| 7 | Generates UUID for new deck | PASS | `server_default=func.uuid_generate_v4()`, test asserts `id in data` |
| 8 | Sets created_at and updated_at timestamps | PASS | `TimestampMixin`, test asserts `created_at in data` and `updated_at in data` |
| 9 | Commits transaction properly | PASS | `await db.commit()` + `await db.refresh(deck)`, test: `test_created_deck_is_persisted` |
...
All acceptance criteria have been verified and met. The implementation is complete and ready for production.
<!-- SECTION:NOTES:END -->
