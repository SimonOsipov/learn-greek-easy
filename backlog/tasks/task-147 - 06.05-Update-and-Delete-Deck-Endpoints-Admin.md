...
- [x] #1 PATCH /api/v1/decks/{id} endpoint implemented
- [x] #2 Returns 200 with updated deck data
- [x] #3 Partial updates work (only provided fields are updated)
- [x] #4 Can update name, description, level, and is_active
- [x] #5 Returns 401 if not authenticated
- [x] #6 Returns 403 if not superuser
- [x] #7 Returns 404 if deck doesn't exist
- [x] #8 Admins can update inactive decks
- [x] #9 Updated_at timestamp is updated

### Delete Endpoint (DELETE)
- [x] #10 DELETE /api/v1/decks/{id} endpoint implemented
- [x] #11 Returns 204 No Content on success
- [x] #12 Performs soft delete (sets is_active=False)
- [x] #13 Does NOT physically delete from database
- [x] #14 Returns 401 if not authenticated
- [x] #15 Returns 403 if not superuser
- [x] #16 Returns 404 if deck doesn't exist
- [x] #17 Can delete already-inactive decks (idempotent)
...
**Branch**: feature/task-147-update-delete-deck-endpoints
**PR**: #23

### Test Results

**Automated Tests**: 57 tests PASSED
- TestCreateDeckEndpoint: 11 tests
- TestSearchDecksEndpoint: 14 tests
- TestGetDeckEndpoint: 6 tests
- TestUpdateDeckEndpoint: 17 tests (NEW)
- TestDeleteDeckEndpoint: 9 tests (NEW)
...
|------|----------|--------|--------|
| Partial update (name only) | 200, name updated, other fields unchanged | 200, confirmed | PASS |
| Full update (all fields) | 200, all fields updated | 200, confirmed | PASS |
| Update inactive deck | 200, admin can update | 200, confirmed | PASS |
| Without auth | 401 Unauthorized | 401 | PASS |
| Non-superuser | 403 Forbidden | 403 | PASS |
| Non-existent deck | 404 Not Found | 404 | PASS |
| Invalid level | 422 Validation Error | 422 | PASS |
| Updated_at changes | Timestamp updated | Confirmed | PASS |

#### DELETE /api/v1/decks/{deck_id}

| Test | Expected | Actual | Status |
|------|----------|--------|--------|
| Delete success | 204 No Content | 204 | PASS |
| Soft delete (is_active=false) | Deck in DB with is_active=false | Confirmed via psql | PASS |
| Deleted deck returns 404 on GET | 404 | 404 | PASS |
| Idempotent (delete twice) | Both return 204 | Both 204 | PASS |
| Without auth | 401 Unauthorized | 401 | PASS |
| Non-superuser | 403 Forbidden | 403 | PASS |
| Non-existent deck | 404 Not Found | 404 | PASS |
| Invalid UUID | 422 Validation Error | 422 | PASS |
...
**Files Modified**:
- `src/api/v1/decks.py` - Added update_deck (lines 324-391) and delete_deck (lines 394-447)
- `src/schemas/deck.py` - DeckUpdate schema (lines 36-42) with optional fields for partial updates
- `tests/integration/api/test_decks.py` - Added TestUpdateDeckEndpoint (17 tests) and TestDeleteDeckEndpoint (9 tests)

**Code Quality**:
- Proper docstrings and type hints
- Follows existing endpoint patterns
- Uses BaseRepository.update() for partial updates
- OpenAPI documentation with response examples
- Error handling consistent with other endpoints

### Result: PASSED

All 17 acceptance criteria met. Implementation is complete and tested.
<!-- SECTION:NOTES:END -->
