...
- [ ] Router file `src/api/v1/decks.py` created with proper structure
- [ ] GET /api/v1/decks endpoint implemented
- [ ] Pagination works correctly (page, page_size query params)
- [ ] Level filter works correctly (optional level query param)
- [ ] Returns DeckListResponse with total count
- [ ] Router registered in v1_router at `/decks` prefix
- [ ] Only active decks are returned (is_active=True)
- [ ] Invalid pagination params return 422 validation error
