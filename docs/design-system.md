...
| `.news-filter-pill-active` | Active state for the news feed country-filter pills (NEWS-07 / NWSR-06). `linear-gradient(180deg, hsl(var(--primary-2)), hsl(var(--primary)))` background + transparent border + white text + `box-shadow: 0 6px 16px -6px hsl(var(--primary) / 0.6)`. Intentionally distinct from `.btn-primary` (different shadow spec). Used in `NewsFilters.tsx`. |
| `.reader-panel` | Frosted rounded-left panel surface for the news in-app reader (`NewsReaderSheet`). `hsl(var(--card))` bg + `backdrop-filter: blur(28px) saturate(180%)` + `border-left: 1px solid hsl(var(--line))` + `border-radius: 22px 0 0 22px` + `var(--shadow-2)` (ADMIN2-46). |
...
| `.fb-*` family | Admin feedback re-skin chrome (card, vote pill, status grid, canned chips, thread bubbles, meta table). Inside the grey `.fb-panel`, the `.fb-card` surface is the recessed CD skin (`bg-2` bg, `--fg/0.06` border, `radius 14px`, hover lift). See `src/index.css` under `@layer components`. Token-only — no raw hex. |
| `.fb-likes` | **CD bordered vote pill** (ADMIN2-45). Wraps the `.fb-card-arrow` (▲, fg-3 12px) + `.fb-vote-count` (mono 13px/700) in a `min-width:44px` recessed box (`hsl(var(--fg)/0.04)` bg + `--fg/0.06` border + `radius 10px`). Replaces the bare arrow+count. `.fb-card-title` (Inter Tight 16/700) + `.fb-card-body` (13.5px fg-2) are also explicitly styled (they previously inherited the Tailwind preflight reset). |
| `.fb-panel` | **Feedback tab contained panel** (ADMIN2-45). Tab-local surface (`bg-2` + `1px hsl(var(--fg)/0.06)` border + `radius 18px + overflow:hidden`) that wraps the toolbar, `.fb-list`, and `.va-panel-foot`. Mirrors `.cl-panel` / `.va-panel.an-panel` approach. Does NOT modify the global transparent `.va-panel`. |
