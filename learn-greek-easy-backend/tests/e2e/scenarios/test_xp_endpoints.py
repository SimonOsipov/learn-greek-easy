...
- GET /api/v1/xp/achievements/unnotified - Unnotified achievements
- POST /api/v1/xp/achievements/notified - Mark achievements as notified
...
class TestUnnotifiedEndpoint(E2ETestCase):
    """E2E tests for GET /api/v1/xp/achievements/unnotified endpoint."""

    @pytest.mark.asyncio
    @pytest.mark.e2e
    @pytest.mark.scenario
    async def test_unnotified_response_structure(
        self, client: AsyncClient, fresh_user_session: UserSession
    ) -> None:
        """Test that unnotified response has correct structure."""
        response = await client.get(
    ...
    @pytest.mark.e2e
    @pytest.mark.scenario
    async def test_unnotified_new_user_empty(
        self, client: AsyncClient, fresh_user_session: UserSession
    ) -> None:
        """Test that new user has no unnotified achievements."""
        response = await client.get(
            "/api/v1/xp/achievements/unnotified",
            headers=fresh_user_session.headers,
        )
        ...
        # New user should have 0 unnotified
        assert data["count"] == 0
        assert len(data["achievements"]) == 0
    ...
    @pytest.mark.e2e
    @pytest.mark.scenario
    async def test_unnotified_unauthenticated_returns_401(self, client: AsyncClient) -> None:
        """Test that unauthenticated request returns 401."""
...
class TestMarkNotifiedEndpoint(E2ETestCase):
    """E2E tests for POST /api/v1/xp/achievements/notified endpoint."""

    @pytest.mark.asyncio
    @pytest.mark.e2e
    @pytest.mark.scenario
    async def test_mark_notified_response_structure(
        self, client: AsyncClient, fresh_user_session: UserSession
    ) -> None:
        """Test that mark notified response has correct structure."""
    ...
    @pytest.mark.e2e
    @pytest.mark.scenario
    async def test_mark_notified_success_response(
        self, client: AsyncClient, fresh_user_session: UserSession
    ) -> None:
        """Test that mark notified returns success."""
        response = await client.post(
            "/api/v1/xp/achievements/notified",
            json={"achievement_ids": ["streak_3", "streak_7"]},
            headers=fresh_user_session.headers,
        )
    ...
    @pytest.mark.e2e
    @pytest.mark.scenario
    async def test_mark_notified_unauthenticated_returns_401(self, client: AsyncClient) -> None:
        """Test that unauthenticated request returns 401."""
        response = await client.post(
            "/api/v1/xp/achievements/notified",
            json={"achievement_ids": ["streak_3"]},
    ...
    @pytest.mark.e2e
    @pytest.mark.scenario
    async def test_mark_notified_empty_list_returns_422(
        self, client: AsyncClient, fresh_user_session: UserSession
    ) -> None:
        """Test that empty achievement_ids list returns 422."""
        response = await client.post(
            "/api/v1/xp/achievements/notified",
    ...
    @pytest.mark.e2e
    @pytest.mark.scenario
    async def test_mark_notified_missing_body_returns_422(
        self, client: AsyncClient, fresh_user_session: UserSession
    ) -> None:
    ...
    @pytest.mark.e2e
    @pytest.mark.scenario
    async def test_mark_notified_idempotent(
        self, client: AsyncClient, fresh_user_session: UserSession
    ) -> None:
        """Test that marking same achievement twice is idempotent."""
        # First call
        ...
        # Second call with same achievement
...
class TestXPEndpointsHTTPMethods(E2ETestCase):
    ...
    @pytest.mark.e2e
    @pytest.mark.scenario
    async def test_unnotified_post_not_allowed(
        self, client: AsyncClient, fresh_user_session: UserSession
    ) -> None:
        """Test that POST to unnotified returns 405."""
        response = await client.post(
            "/api/v1/xp/achievements/unnotified",
            headers=fresh_user_session.headers,
        )
    ...
    @pytest.mark.e2e
    @pytest.mark.scenario
    async def test_mark_notified_get_not_allowed(
        self, client: AsyncClient, fresh_user_session: UserSession
    ) -> None:
        """Test that GET to notified returns 405."""
        response = await client.get(
            "/api/v1/xp/achievements/notified",
            headers=fresh_user_session.headers,
        )
