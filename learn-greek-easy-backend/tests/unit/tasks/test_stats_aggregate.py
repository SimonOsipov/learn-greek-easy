...
class TestStatsAggregateTaskExecution:
    ...
    @pytest.mark.asyncio
    async def test_stats_aggregate_uses_shared_session_factory(self):
        """Test that stats_aggregate_task uses get_session_factory (shared pool)."""
        from src.tasks.scheduled import stats_aggregate_task
        ...
        # Verify get_session_factory was called (not create_async_engine)
...
class TestStatsAggregateSessionCleanupOrder:
    """Test that session context manager is used correctly.

    After migration to get_session_factory(), the async context manager handles
    session lifecycle automatically — there is no explicit session.close() or
    engine.dispose() to order. These tests verify context manager enter/exit.
    """

    @pytest.mark.asyncio
    async def test_stats_aggregate_exits_session_on_success(self):
        """Test that session context is exited after successful execution."""
        from src.tasks.scheduled import stats_aggregate_task
    ...
    @pytest.mark.asyncio
    async def test_stats_aggregate_exits_session_on_error(self):
