...
NOTE: These tests require the NGEN-10-01 data model (WordEntry with owner_id/visibility,
DeckWordEntry junction table). They will be updated when the factory/fixture support
for DeckWordEntry is available.
"""
...
async def _associate_entry_with_deck(
    db_session: AsyncSession,
    entry: WordEntry,
    deck_id,
) -> None:
    """Create a DeckWordEntry junction record linking an entry to a deck."""
    junction = DeckWordEntry(deck_id=deck_id, word_entry_id=entry.id)
...
class TestDuplicateDetectionExactMatch:
    ...
    async def test_exact_match_same_lemma_pos(self, db_session: AsyncSession):
        ...
        entry = await _create_word_entry(db_session, "σπίτι", PartOfSpeech.NOUN)
        ...
        assert result.matched_decks[0].deck_id == deck.id
        assert result.matched_decks[0].deck_name == deck.name_en
...
class TestDuplicateDetectionAccentInsensitive:
    ...
    async def test_accent_insensitive_reverse(self, db_session: AsyncSession):
        ...
        deck = await DeckFactory.create(session=db_session)
        entry = await _create_word_entry(db_session, "σπιτι", PartOfSpeech.NOUN)
        await _associate_entry_with_deck(db_session, entry, deck.id)
...
class TestDuplicateDetectionExcludeDeck:
    ...
    async def test_exclude_deck_id_skips_target(self, db_session: AsyncSession):
        ...
        entry = await _create_word_entry(db_session, "σπίτι", PartOfSpeech.NOUN)
        await _associate_entry_with_deck(db_session, entry, deck_a.id)
    ...
    async def test_exclude_deck_id_finds_other_decks(self, db_session: AsyncSession):
        """Entry in decks A and B, exclude A → finds entry in B → is_duplicate=True."""
        deck_a = await DeckFactory.create(session=db_session)
        deck_b = await DeckFactory.create(session=db_session)
        entry = await _create_word_entry(db_session, "σπίτι", PartOfSpeech.NOUN)
        await _associate_entry_with_deck(db_session, entry, deck_a.id)
        await _associate_entry_with_deck(db_session, entry, deck_b.id)
        ...
        assert result.is_duplicate is True
        assert len(result.matched_decks) == 1
        assert result.matched_decks[0].deck_id == deck_b.id
...
class TestDuplicateDetectionInactiveEntries:
    ...
    async def test_inactive_entry_excluded(self, db_session: AsyncSession):
        ...
        deck = await DeckFactory.create(session=db_session)
        entry = await _create_word_entry(db_session, "σπίτι", PartOfSpeech.NOUN, is_active=False)
        await _associate_entry_with_deck(db_session, entry, deck.id)
