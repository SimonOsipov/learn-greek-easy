...
- Seeds via test_factory.begin() so the pipeline's own factory.begin() sessions can
  see the committed rows (both use the same engine; real commits are visible).
- Assertions on DB state re-load the row via test_factory after the pipeline
  completes.
"""
...
from src.db.models import DescriptionStatus, Situation, SituationDescription, SituationStatus
...
async def _seed(
    ...
    """Seed a Situation + SituationDescription via factory with a real commit.

    Returns (situation_id, description_id).
    """
    async with factory.begin() as session:
        situation = SituationFactory.build(**situation_kwargs)
        session.add(situation)
        await session.flush()
        desc_kwargs = {**description_kwargs, "situation_id": situation.id}
        description = SituationDescriptionFactory.build(**desc_kwargs)
        session.add(description)
        await session.flush()
        situation_id = situation.id
        description_id = description.id
    # context exit commits
    return situation_id, description_id
...
async def _cleanup(
    ...
    """Remove seeded rows explicitly after test."""
    async with factory.begin() as session:
        await session.execute(
            delete(SituationDescription).where(SituationDescription.id == description_id)
        )
        await session.execute(delete(Situation).where(Situation.id == situation_id))
...
class TestRunDescriptionAudioPipelineB1:
    ...
    async def test_pipeline_writes_b1_columns(
        ...
        situation_id, description_id = await _seed(
            test_factory,
            situation_kwargs={"status": SituationStatus.READY},
            description_kwargs={
        ...
        try:
            ...
            await _cleanup(test_factory, situation_id, description_id)
