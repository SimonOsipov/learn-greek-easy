...
This is a **release handshake**, not agent-driven browsing: RALPH triggers the locked `release-verify.yml`, waits for it, and consumes its verification artifacts. It does NOT resolve a preview URL or drive Playwright/Maestro against shared dev directly — all verification runs inside the `dev-release-lease` window so parallel stories never stomp the shared dev environment. **One pilot exception:** step 5's CD-fidelity capture browses the deployed dev env read-only under a mandatory SHA pin (see step 5's guardrails).
...
5. **Whole-surface CD-fidelity capture (PILOT — stories with rendered-UI changes only; skip otherwise).** The locked-release artifact covers only fixed EN surfaces (login/dashboard/decks) — for the surfaces the story actually touched, capture evidence directly. This is the ONE sanctioned exception to the "no agent browsing against shared dev" rule, bounded by these guardrails:
   - **SHA pin (mandatory):** before AND after capture, fetch the deployed dev frontend's `/version` and assert it matches the PR head SHA (`git -C "$WORKTREE_PATH" rev-parse HEAD`). Mismatch at either end (a parallel story's release re-deployed mid-capture) → discard ALL captures, re-trigger the release, retry once. Never judge pixels you cannot pin to the commit.
   - **Capture set:** every surface the story touched — **including interactive states** (drawers, modals, toggles, filter bars), not just landing routes — in **EN and RU**, full-page, via the standalone Playwright MCP authed as the seeded user/admin. Save to `$WORKTREE_PATH/.ralph/fidelity/<surface>-<locale>-<state>.png`.
   - **Reference = the CD prototype, pulled fresh:** fetch the prototype files backing each surface from the Prototypes project (`019e1856-774d-7795-8b57-fbba04550fc7`) and confirm the file→surface mapping FIRST (admin and learner screens share names — see `feedback_admin_cd_fidelity_qa.md`). Diff live `getComputedStyle` (and `getBoundingClientRect` where layout is in question) against the prototype's actual CSS rules — never against memory or the story's prose.
   - **Output:** `$WORKTREE_PATH/.ralph/fidelity/deltas.md` — a delta-ONLY per-element table (element, property, live value, prototype value). A delta citing a design-system rule or a prototype CSS rule is a real fail; uncited taste remains advisory (see Style in step 6).
   - **Evidence rule (mechanical):** for a rendered-UI story, missing `.ralph/fidelity/` captures or `deltas.md` = this gate did not run — Phase 3.5 cannot PASS.
   - *Pilot note:* after ~3 UI stories, evaluate promoting the static route×locale captures into `release-web-verify.cjs` (in-CI artifact) and keeping agent capture for interactive states only (loop-analysis-v2 rec P1; signal-environment prerequisite is PERF-23).
6. **Spawn `product-qa-spec`** (default critique disposition). For **each** acceptance criterion, emit `pass | fail` **plus a screenshot as evidence — from the consumed locked-release artifact, or (for surfaces the artifact doesn't cover) from the SHA-pinned step-5 fidelity captures**. A holistic "looks done" is not allowed — every AC needs its own pixel proof from one of those two evidence channels. For rendered-UI stories, also review `deltas.md`: every real (rule-citing) delta joins the fails batch alongside failing ACs — per-AC verdicts and the fidelity table are BOTH required; neither substitutes for the other.
...
| Driving Playwright/Maestro against shared dev from the agent, outside the lease | Use the Phase 3.5 release handshake — trigger `release-verify.yml` (label or `--ref`), wait, consume its artifacts. ONE exception: Phase 3.5 step 5's read-only, SHA-pinned CD-fidelity capture |
