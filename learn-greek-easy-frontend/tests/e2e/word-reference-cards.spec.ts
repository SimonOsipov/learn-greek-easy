...
test.describe('Word Reference - Cards Tab', () => {
  test.beforeAll(async ({ request }) => {
    ...
    const v2Deck = (decksData.decks ?? []).find(
      (d: { name: string }) => d.name === 'E2E V2 Nouns'
    );
    if (!v2Deck) {
      throw new Error('[WCRD] Could not find "E2E V2 Nouns" deck. Ensure seed data is loaded.');
    }
    ...
    if (!firstWord) {
      throw new Error('[WCRD] No word entries found in E2E V2 Nouns deck.');
    }
