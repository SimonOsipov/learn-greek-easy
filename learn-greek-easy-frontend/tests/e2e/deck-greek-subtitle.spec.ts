...
// ---------------------------------------------------------------------------
// SUITE 3: Equal-case — Greek subtitle hidden when name_el === localizedTitle
//
// This proves the dedup guard at the deck-detail hero level.
// We test this via API route mocking (no seed data has name_el === name_en).
// ---------------------------------------------------------------------------

test.describe('DGREEK-08: Equal-case dedup guard via API mock', () => {
  const MOCK_DECK_ID = 'mock-equal-deck-id';

  const MOCK_DECK_RESPONSE = {
    id: MOCK_DECK_ID,
    name: 'Numbers',
    name_en: 'Numbers',
    name_el: 'Numbers',    // equal to name_en → subtitle must be suppressed
    name_ru: 'Числа',
    description: 'Numbers deck',
    description_en: 'Numbers deck',
    description_ru: 'Колода чисел',
    description_el: null,
    level: 'A1',
    is_active: true,
    is_premium: false,
  ...
  const MOCK_DECK_LIST = {
    total: 1,
    page: 1,
    page_size: 100,
    decks: [MOCK_DECK_RESPONSE],
  };
  ...
  const MOCK_PROGRESS: Record<string, unknown> = {};

  test('en locale: index card hides Greek subtitle when name_el === name_en', async ({ page }) => {
    // Mock the deck list endpoint to return a deck where name_el === name_en
    await page.route('**/api/v1/decks**', (route) => {
      route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify(MOCK_DECK_LIST),
      });
    });
    ...
    // EN title shown
    await expect(card.locator('[data-testid="deck-card-title"]')).toHaveText('Numbers', {
      timeout: 10000,
    });

    // Greek subtitle MUST NOT be rendered (name_el === name, so titleGreek === title)
    const greekSubtitle = card.locator('[data-testid="deck-card-greek-subtitle"]');
    await expect(greekSubtitle).not.toBeVisible();
  });
});
